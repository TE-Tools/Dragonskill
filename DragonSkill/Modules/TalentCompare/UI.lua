-- Dragon Skill - Haupt UI (v1.5.9)
-- Spec-Skillungen, Minimap, /ds help + boss
...
local function CreateMinimapButton()
    ...
    local icon = btn:CreateTexture(nil, "ARTWORK"); icon:SetSize(20, 20); icon:SetPoint("CENTER", 0, 1)
    icon:SetTexture("Interface\\Icons\\Inv_misc_head_dragon_01"); btn.icon = icon
    local border = btn:CreateTexture(nil, "OVERLAY"); border:SetSize(54, 54); border:SetPoint("TOPLEFT")
    ...
    btn:SetScript("OnClick", function(_, button)
        ...
        else print("|cff00ff00Dragon Skill:|r /ds help") end
    end)
    ...
    btn:SetScript("OnEnter", function(self)
        ...
        GameTooltip:AddLine("Dragon Skill"); GameTooltip:AddLine("|cffaaaaaaL: Fenster · Ziehen: Position|r")
        GameTooltip:Show()
    end)
    btn:SetScript("OnLeave", function() GameTooltip:Hide() end)
    minimapBtn = btn; UpdateMinimapPosition()
    if DragonSkillDB and DragonSkillDB.minimap and DragonSkillDB.minimap.hide then btn:Hide() else btn:Show() end
end
...
function UI:ToggleMinimap()
    ...
    if DragonSkillDB.minimap.hide then minimapBtn:Hide(); print("|cff00ff00Dragon Skill:|r Minimap aus")
    else minimapBtn:Show(); print("|cff00ff00Dragon Skill:|r Minimap an") end
end

function UI:Init()
    if self.frame then return end
    local f = CreateFrame("Frame", "DragonSkillMainFrame", UIParent, "ButtonFrameTemplate")
    f:SetSize(FRAME_WIDTH, FRAME_HEIGHT); f:SetPoint("CENTER")
    f:SetMovable(true); f:EnableMouse(true); f:SetClampedToScreen(true)
    f:RegisterForDrag("LeftButton")
    f:SetScript("OnDragStart", f.StartMoving); f:SetScript("OnDragStop", f.StopMovingOrSizing)
    if f.SetTitle then f:SetTitle("Dragon Skill v" .. (DragonSkill.version or "1.5.9") .. SpecTitleSuffix()) end
    if f.portrait then f.portrait:SetTexture("Interface\\Icons\\Inv_misc_head_dragon_01") end
    if f.Inset then
        f.Inset:ClearAllPoints(); f.Inset:SetPoint("TOPLEFT", 4, -60); f.Inset:SetPoint("BOTTOMRIGHT", -6, 28)
    end
    local scrollFrame = CreateFrame("ScrollFrame", "DragonSkillScrollFrame", f.Inset, "UIPanelScrollFrameTemplate")
    scrollFrame:SetPoint("TOPLEFT", 8, -8); scrollFrame:SetPoint("BOTTOMRIGHT", -28, 8); scrollFrame:EnableMouse(true)
    local content = CreateFrame("Frame", "DragonSkillContentFrame", scrollFrame)
    content:SetSize(CONTENT_WIDTH, 2000); content:EnableMouse(true)
    scrollFrame:SetScrollChild(content); f.Content = content; f.ScrollFrame = scrollFrame
    f.Tabs = {}
    ...
    if f.EnableKeyboard then
        ...
        f:SetScript("OnKeyDown", function(selfFrame, key)
            if key == "ESCAPE" then selfFrame:SetPropagateKeyboardInput(false); selfFrame:Hide()
            else selfFrame:SetPropagateKeyboardInput(true) end
        end)
    end
    tinsert(UISpecialFrames, "DragonSkillMainFrame")
    f:Hide(); self.frame = f; self.talentBtns = {}; self.favBtns = {}; self.listRowsByTab = {}
end
...
function UI:ClearContent(content)
    ...
    if content.extraFS then for _, fs in ipairs(content.extraFS) do fs:Hide() end; wipe(content.extraFS) end
...
function UI:EnsureText(content)
    if not content.text then
        ...
        content.text:SetWidth(CONTENT_WIDTH - 30); content.text:SetJustifyH("LEFT"); content.text:SetSpacing(3)
    else content.text:SetWidth(CONTENT_WIDTH - 30) end
    return content.text
end
...
function UI:Update()
    ...
    if self.frame.SetTitle then
        self.frame:SetTitle("Dragon Skill v" .. (DragonSkill.version or "1.5.9") .. SpecTitleSuffix())
    end
    local content = self.frame.Content
    self:ClearContent(content); self:EnsureText(content)
    ...
    elseif currentTab == 9 then self:DrawBuffs(content, guideData) end
end
...
local function ShowDiffForBuild(build, TC)
    ...
    StaticPopup_Show("DRAGONSKILL_DIFF", string.format("|cffffff00Diff: %s|r\n\n%s%s", tostring(build.label or "Build"), modeHint, summary))
end
...
function UI:DrawTalents(content, guideData)
    ...
    for _, build in ipairs(raw) do
        ...
        if key ~= "" and not seen[key] then
            ...
            table.insert(builds, { importString = build.importString, provider = build.provider, label = build.label, _sim = r.similarity or 0 })
        end
    end
    ...
    if #builds == 0 then
        ...
        for i, build in ipairs(builds) do
            ...
            if not btn then
                ...
                btn:SetSize(ROW_WIDTH, 32); btn:RegisterForClicks("LeftButtonUp", "RightButtonUp"); self.talentBtns[i] = btn
            end
            btn:SetParent(content); btn:ClearAllPoints(); btn:SetPoint("TOPLEFT", 10, yOffset)
            local sim = build._sim or 0
            local simColor = sim >= 90 and "00ff00" or (sim >= 70 and "ffff00" or "ff6666")
            local star = (i == 1 and sim >= 90) and "★ " or ""
            btn:SetText(string.format("%s[%s] %s  |cff%s%d%%|r", star, string.upper(tostring(build.provider or "Guide")), tostring(build.label or ("Build " .. i)), simColor, sim))
            ...
            btn:Show(); yOffset = yOffset - 36
        end
    end
    yOffset = yOffset - 12
    local favHeader = content:CreateFontString(nil, "OVERLAY", "GameFontNormal")
    favHeader:SetPoint("TOPLEFT", 10, yOffset)
    favHeader:SetText("|cffffff00=== Meine Skillungen (diese Spec) ===|r"); favHeader:Show()
    table.insert(content.extraFS, favHeader); yOffset = yOffset - 22
    ...
    if #skillings == 0 then
        ...
        empty:SetPoint("TOPLEFT", 10, yOffset); empty:SetWidth(ROW_WIDTH); empty:SetJustifyH("LEFT")
        empty:SetText("|cff888888Keine Skillungen für diese Spec.|r"); empty:Show()
        table.insert(content.extraFS, empty); yOffset = yOffset - 40
    else
        for i, entry in ipairs(skillings) do
            ...
            if not btn then
                ...
                btn:SetSize(ROW_WIDTH - 70, 28); btn:RegisterForClicks("LeftButtonUp", "RightButtonUp"); self.favBtns[i] = btn
            end
            btn:SetParent(content); btn:ClearAllPoints(); btn:SetPoint("TOPLEFT", 10, yOffset)
            ...
            btn:SetScript("OnClick", function(_, mouseButton)
                local build = { importString = data.importString, provider = data.provider or "saved", label = data.label or name }
                cachedBuildData = build
                if mouseButton == "RightButton" then ShowDiffForBuild(build, TC)
            ...
            del:SetParent(content); del:ClearAllPoints(); del:SetPoint("LEFT", btn, "RIGHT", 4, 0)
            del:SetText("X"); del:SetScript("OnClick", function() StaticPopup_Show("DRAGONSKILL_DELETE", name, nil, name) end)
            del:Show(); yOffset = yOffset - 32
        end
    end
...
function UI:Helper_DrawListWithIcons(content, items, title)
    local yOffset = -10; self:EnsureText(content); items = FilterItems(items)
    ...
    for _, row in ipairs(rows) do row:Hide(); row:SetScript("OnEnter", nil); row:SetScript("OnLeave", nil) end
    for tabId, otherRows in pairs(self.listRowsByTab) do
        if tabId ~= currentTab then for _, row in ipairs(otherRows) do row:Hide() end end
    end
    if not items or #items == 0 then content.text:SetText((title or "") .. "\n\n|cffffaa00Keine Daten.|r"); return end
    for i, item in ipairs(items) do
        local itemName = item.text or item.name or "Unbekannt"
        local row = rows[i]
        if not row then
            row = CreateFrame("Button", nil, content); row:SetSize(ROW_WIDTH, 26); row:EnableMouse(true)
            row.icon = row:CreateTexture(nil, "ARTWORK"); row.icon:SetSize(22, 22); row.icon:SetPoint("LEFT", 0, 0)
            row.text = row:CreateFontString(nil, "OVERLAY", "GameFontHighlight")
            row.text:SetPoint("LEFT", row.icon, "RIGHT", 8, 0); row.text:SetJustifyH("LEFT"); row.text:SetWidth(ROW_WIDTH - 40)
            rows[i] = row
        end
        row:SetParent(content); row:ClearAllPoints(); row:SetPoint("TOPLEFT", 10, yOffset)
        row:SetFrameLevel(content:GetFrameLevel() + 5)
        local texture = "Interface\\Icons\\Inv_misc_questionmark"
        if item.itemId and C_Item and C_Item.GetItemIconByID then texture = C_Item.GetItemIconByID(item.itemId) or texture
        elseif item.spellId and C_Spell and C_Spell.GetSpellTexture then texture = C_Spell.GetSpellTexture(item.spellId) or texture end
...
function UI:DrawStats(content, guideData)
    ...
    if SP and SP.GetForCurrentSpec then local spData = SP:GetForCurrentSpec(); if spData then prio = spData end end
    if prio then
        txt = txt .. "\n|cffffff00=== Empfehlung ===|r\n|cffffd100Wowhead:|r " .. (prio.wowhead or "N/A") .. "\n"
...
function UI:DrawTrinkets(content, guideData)
    ...
    self:Helper_DrawListWithIcons(content, (mod and mod.GetList and mod:GetList(guideData)) or {}, "|cffffff00Top Trinkets (Archon):|r")
end
...
function UI:DrawRotation(content, guideData)
    ...
    self:Helper_DrawListWithIcons(content, (mod and mod.GetList and mod:GetList(guideData)) or {}, "|cffffff00Priorität (Wowhead):|r\n|cff888888Nur Anzeige.|r")
end
function UI:DrawGear(content, guideData)
    self:Helper_DrawListWithIcons(content, (guideData.bisGear and guideData.bisGear.wowhead) or {}, "|cffffff00Best-in-Slot:|r")
end
function UI:DrawEnchants(content, guideData)
    self:Helper_DrawListWithIcons(content, (guideData.enchants and guideData.enchants.wowhead) or {}, "|cffffff00Verzauberungen:|r")
end
function UI:DrawGems(content, guideData)
    self:Helper_DrawListWithIcons(content, (guideData.gems and guideData.gems.wowhead) or {}, "|cffffff00Gems:|r")
end
function UI:DrawBuffs(content, guideData)
    self:Helper_DrawListWithIcons(content, (guideData.consumables and guideData.consumables.wowhead) or {}, "|cffffff00Buffs / Consumables:|r")
end
...
local function PrintHelp()
    ...
    print("  /ds boss <name|id>         – Boss-Mechanik testen")
    print("  Beispiele: /ds boss ulatek · /ds boss nekzali · /ds boss nymrissa")
end
...
SlashCmdList["WEAR"] = function(msg)
    ...
    if bossArg ~= nil then
        ...
        if not BM then print("|cffff0000Dragon Skill:|r BossMechanics nicht geladen."); return end
...
print("|cff00ff00Dragon Skill v" .. (DragonSkill.version or "1.5.9") .. " geladen!|r  /ds help")

DragonSkill.Events:On("PLAYER_LOGIN", function()
    UI:Init(); CreateMinimapButton()
end)
DragonSkill.Events:On("PLAYER_SPECIALIZATION_CHANGED", function()
    if C_Timer and C_Timer.After then C_Timer.After(0.15, RefreshIfShown) else RefreshIfShown() end
end)
