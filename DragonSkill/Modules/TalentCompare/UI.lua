...
local SLOT_ORDER = {
    Head = 1, Neck = 2, Shoulder = 3, Shoulders = 3, Back = 4, Cloak = 4,
    Chest = 5, Wrist = 6, Hands = 7, Gloves = 7, Waist = 8, Belt = 8,
    Legs = 9, Feet = 10, Boots = 10, Finger = 11, Ring = 11, Trinket = 12,
    MainHand = 13, Weapon = 13, OffHand = 14, Shield = 14,
}

local function RankValue(rank)
    if not rank then return 0 end
    if type(rank) == "number" then return rank end
    local s = tostring(rank):gsub("%%", ""):gsub(",", ".")
...
SlashCmdList["WEAR"] = function(msg)
    ...
    if bossArg ~= nil then
        ...
        if bossArg == "" or bossArg == "list" then
